/**
 * Vehicle Development - ORM MVC DRS with REST on Express
...
if (appdir === false) {
    console.log("No Vehicle App Selected. Try vehicle start app");
    process.exit();
}
...
console.log("Application Loaded", wheels);
...
if (config_api.public) {
  ...
  orm.initialize(config_orm, function(err, models) {
    ...
    app.listen(config.port, function() {
      ...
      vehicle.emit('ready');
      if (require(appdir +'/configs/'+ env +'/openurl.js').openurl) {
        console.log("Opening Localhost.Rocks!");
        if (config.port!=80) {
          openurl.open('http://localhost.rocks:' + config.port);
        } else {  
          openurl.open('http://localhost.rocks');
        }
      }
    });
  });
